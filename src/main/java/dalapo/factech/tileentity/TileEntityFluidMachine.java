...
public abstract class TileEntityFluidMachine extends TileEntityMachine implements IHasFluid
{
	...
	public void showChatInfo(EntityPlayer ep)
	{
		...
		{
			str = "Tank " + (i+1) + ": ";
			if (tanks[i].getFluid() == null) str += "Empty";
			else
			{
				str += tanks[i].getFluid().getLocalizedName() + ", " + tanks[i].getFluidAmount();
			}
	...
	public void readFromNBT(NBTTagCompound nbt)
	{
		...
		if (nbt.hasKey("tanks"))
		{
			...
			for (int i=0; i<tankList.tagCount(); i++)
			{
				...
				if (tankInfo.getString("name").equals("[empty]"))
				{
