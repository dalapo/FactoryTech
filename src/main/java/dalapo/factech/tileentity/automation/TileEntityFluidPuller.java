...
public class TileEntityFluidPuller extends TileEntity implements ITickable {
	...
	@Override
	public void update() {
		if (!world.getBlockState(getPos()).getBlock().equals(BlockRegistry.fluidPuller)) return;
				...
				pushing != null && pushing.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, front.getOpposite()))
		{
			...
			int space = 0;
			if (pushing instanceof TileEntityPipe)
			{
				space = ((TileEntityPipe)pushing).insertFluid(toMove.getFluid(), toMove.amount, false);
			}
			else space = pushTank.fill(toMove.copy(), false);
			...
			if (space > 0)
			{
				...
				if (pushing instanceof TileEntityPipe)
				{
					((TileEntityPipe)pushing).insertFluid(toMove.getFluid(), toMove.amount, true);
				}
				else pushTank.fill(toMove.copy(), true);
			}
		}
	}
